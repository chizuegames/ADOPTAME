...
/* Hotspot sobre la nueva flecha ubicada en la parte inferior izquierda de IMGRE. */
const threatNextHotspot = document.createElement("button");
threatNextHotspot.type = "button";
threatNextHotspot.setAttribute("aria-label", "Continuar a la siguiente pantalla");
...
renderScene = function () {
  ...
  if (scene.type === "threat-card") {
    ...
    stage.setAttribute(
      "aria-label",
      "Tarjeta de amenaza interactiva. Toca una zona de la tarjeta para conocer su función o usa la flecha inferior izquierda para continuar."
    );
  }
};

/* Evita que IMGRE avance tocando cualquier punto: se avanza con su flecha. */
...
threatLayer.addEventListener("click", event => {
  ...
  if (button) {
    explainThreat(button.dataset.threat);
  }
});
